// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using SMS_backend;

#nullable disable

namespace SMS_backend.Migrations
{
    [DbContext(typeof(Db))]
    [Migration("20251222051756_5")]
    partial class _5
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("SMS_backend.Models.Author", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("AuthorStatus")
                        .HasColumnType("int");

                    b.Property<string>("AuthorityName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Biography")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("CreatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("CreatorID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("DateOfBirth")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DateOfDeath")
                        .HasColumnType("datetime2");

                    b.Property<string>("FirstName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nationality")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("VIAF_ID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("WebSiteURL")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ID");

                    b.HasIndex("CreatorID");

                    b.ToTable("Authors");
                });

            modelBuilder.Entity("SMS_backend.Models.AuthorLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("AuthorID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("AuthorID");

                    b.HasIndex("UpdaterID");

                    b.ToTable("AuthorLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Book", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("AuthorID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("CreatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("CreatorID")
                        .HasColumnType("int");

                    b.Property<string>("DeweyDecimal")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EditionNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Format")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ISBN10")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ISBN13")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ImageURL")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool?>("IsDigital")
                        .HasColumnType("bit");

                    b.Property<string>("LanguageCode")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PublicationYear")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("PublisherID")
                        .HasColumnType("int");

                    b.Property<int?>("RecordStatus")
                        .HasColumnType("int");

                    b.Property<string>("SubTitle")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Summary")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Title")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("TotalPages")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("AuthorID");

                    b.HasIndex("CreatorID");

                    b.HasIndex("PublisherID");

                    b.ToTable("Books");
                });

            modelBuilder.Entity("SMS_backend.Models.BookLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("BookID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("BookID");

                    b.HasIndex("UpdaterID");

                    b.ToTable("BookLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Category", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<DateTime?>("CreatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("CreatorID")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("RecordStatus")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("CreatorID");

                    b.ToTable("Categories");
                });

            modelBuilder.Entity("SMS_backend.Models.CategoryLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("CategoryID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("CategoryID");

                    b.HasIndex("UpdaterID");

                    b.ToTable("CategoryLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Classroom", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("AdviserID")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("SchoolYear")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ID");

                    b.HasIndex("AdviserID");

                    b.ToTable("Classroom");
                });

            modelBuilder.Entity("SMS_backend.Models.ClassroomLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("ClassroomID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("ClassroomID");

                    b.HasIndex("UpdaterID");

                    b.ToTable("ClassroomLog");
                });

            modelBuilder.Entity("SMS_backend.Models.DailyConsumption", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<DateTime?>("ApprovedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("ApproverID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("BuyOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("BuyerID")
                        .HasColumnType("int");

                    b.Property<int?>("ProductConsumptionStatus")
                        .HasColumnType("int");

                    b.Property<int?>("ProductID")
                        .HasColumnType("int");

                    b.Property<int?>("ProductInventoryID")
                        .HasColumnType("int");

                    b.Property<int?>("Quantity")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("ApproverID");

                    b.HasIndex("BuyerID");

                    b.HasIndex("ProductID");

                    b.HasIndex("ProductInventoryID");

                    b.ToTable("DailyConsumptions");
                });

            modelBuilder.Entity("SMS_backend.Models.DailyConsumptionLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("DailyConsumptionID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("DailyConsumptionID");

                    b.HasIndex("UpdaterID");

                    b.ToTable("DailyConsumptionLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Department", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<DateTime?>("CreatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("CreatorID")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("RecordStatus")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("CreatorID");

                    b.ToTable("Departments");

                    b.HasData(
                        new
                        {
                            ID = 1,
                            Name = "Management",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 2,
                            Name = "Research & Community Extension",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 3,
                            Name = "School Chaplain",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 4,
                            Name = "Human Resource Department",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 5,
                            Name = "Admission",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 6,
                            Name = "Academic Department",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 7,
                            Name = "Technical/Vocational (TechVoc) Program",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 8,
                            Name = "Guidance & Counseling",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 9,
                            Name = "Learning Resource Center (LRC)",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 10,
                            Name = "Student Affairs & Services Office (OSA)",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 11,
                            Name = "Pre-School / Elementary / Junior High School",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 12,
                            Name = "Preschool",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 13,
                            Name = "Elementary",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 14,
                            Name = "Junior High School (JHS)",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 15,
                            Name = "Senior High School (SHS)",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 16,
                            Name = "Grade 11",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 17,
                            Name = "Grade 12",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 18,
                            Name = "Administration Department",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 19,
                            Name = "ICT / Admin Officer",
                            RecordStatus = 1
                        });
                });

            modelBuilder.Entity("SMS_backend.Models.DepartmentLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("DepartmentID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("DepartmentID");

                    b.HasIndex("UpdaterID");

                    b.ToTable("DepartmentLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Position", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<DateTime?>("CreatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("CreatorID")
                        .HasColumnType("int");

                    b.Property<int?>("DepartmentID")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("RecordStatus")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("CreatorID");

                    b.HasIndex("DepartmentID");

                    b.ToTable("Positions");

                    b.HasData(
                        new
                        {
                            ID = 1,
                            DepartmentID = 1,
                            Name = "School Director",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 2,
                            DepartmentID = 2,
                            Name = "Head",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 3,
                            DepartmentID = 3,
                            Name = "Chaplain",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 4,
                            DepartmentID = 4,
                            Name = "HR Dept Head",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 5,
                            DepartmentID = 5,
                            Name = "Admission & External Relations Officer",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 6,
                            DepartmentID = 6,
                            Name = "Registrar",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 7,
                            DepartmentID = 7,
                            Name = "TechVoc Program Head",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 8,
                            DepartmentID = 8,
                            Name = "Guidance & Counseling Center Head",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 9,
                            DepartmentID = 9,
                            Name = "Head",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 10,
                            DepartmentID = 10,
                            Name = "Head",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 11,
                            DepartmentID = 11,
                            Name = "Academic Head, Pre-School / JHS",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 12,
                            DepartmentID = 12,
                            Name = "Preschool Coordinator",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 13,
                            DepartmentID = 13,
                            Name = "Elementary Coordinator",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 14,
                            DepartmentID = 14,
                            Name = "JHS Coordinator",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 15,
                            DepartmentID = 15,
                            Name = "SHS Principal",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 16,
                            DepartmentID = 16,
                            Name = "Grade 11 Coordinator",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 17,
                            DepartmentID = 17,
                            Name = "Grade 12 Coordinator",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 18,
                            DepartmentID = 18,
                            Name = "Administration Dept Head",
                            RecordStatus = 1
                        },
                        new
                        {
                            ID = 19,
                            DepartmentID = 19,
                            Name = "ICT Admin Officer",
                            RecordStatus = 1
                        });
                });

            modelBuilder.Entity("SMS_backend.Models.PositionLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("PositionID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("PositionID");

                    b.HasIndex("UpdaterID");

                    b.ToTable("PositionLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Product", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("CategoryID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("CreatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("CreatorID")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("RecordStatus")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("CategoryID");

                    b.HasIndex("CreatorID");

                    b.ToTable("Products");
                });

            modelBuilder.Entity("SMS_backend.Models.ProductInventory", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<DateTime?>("CreatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("CreatorID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("DateInventory")
                        .HasColumnType("datetime2");

                    b.Property<string>("Price")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("ProductID")
                        .HasColumnType("int");

                    b.Property<int?>("ProductInventoryStatus")
                        .HasColumnType("int");

                    b.Property<int?>("Quantity")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("CreatorID");

                    b.HasIndex("ProductID");

                    b.ToTable("ProductInventories");
                });

            modelBuilder.Entity("SMS_backend.Models.ProductInventoryLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("ProductInventoryID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("ProductInventoryID");

                    b.HasIndex("UpdaterID");

                    b.ToTable("ProductInventoryLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.ProductLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("ProductID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("ProductID");

                    b.HasIndex("UpdaterID");

                    b.ToTable("ProductLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Publisher", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<string>("AddressLine1")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("City")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ContactEmail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ContactPerson")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ContactPhone")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Country")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("CreatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("CreatorID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("DateEstablished")
                        .HasColumnType("datetime2");

                    b.Property<string>("InternalNotes")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PostalCode")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("RecordStatus")
                        .HasColumnType("int");

                    b.Property<string>("ShortCode")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("StateProvince")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ID");

                    b.HasIndex("CreatorID");

                    b.ToTable("Publishers");
                });

            modelBuilder.Entity("SMS_backend.Models.PublisherLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("PublisherID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("PublisherID");

                    b.HasIndex("UpdaterID");

                    b.ToTable("PublisherLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.RefreshToken", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<DateTime?>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("ExpiresAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("IPAddress")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("JwtID")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ReplacedByToken")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("RevokedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Token")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserAgent")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("UserID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("UserID");

                    b.ToTable("RefreshTokens");
                });

            modelBuilder.Entity("SMS_backend.Models.Role", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<DateTime?>("CreatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("CreatorID")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("RecordStatus")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("CreatorID");

                    b.ToTable("Roles");
                });

            modelBuilder.Entity("SMS_backend.Models.RoleLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("RoleID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("RoleID");

                    b.HasIndex("UpdaterID");

                    b.ToTable("RoleLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Subject", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<string>("Day")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Hours")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("TeacherID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("TeacherID");

                    b.ToTable("Subject");
                });

            modelBuilder.Entity("SMS_backend.Models.SubjectLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<int?>("SubjectID")
                        .HasColumnType("int");

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("SubjectID");

                    b.HasIndex("UpdaterID");

                    b.ToTable("SubjectLog");
                });

            modelBuilder.Entity("SMS_backend.Models.User", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<string>("Avatar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("CreatedOn")
                        .HasColumnType("datetime2");

                    b.Property<string>("FirstName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Password")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("PositionID")
                        .HasColumnType("int");

                    b.Property<int?>("RecordStatus")
                        .HasColumnType("int");

                    b.Property<string>("Username")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ID");

                    b.HasIndex("PositionID");

                    b.ToTable("Users");
                });

            modelBuilder.Entity("SMS_backend.Models.UserLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.Property<int?>("UserID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("UpdaterID");

                    b.HasIndex("UserID");

                    b.ToTable("UserLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.UserRole", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<DateTime?>("AssignedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("AssignerID")
                        .HasColumnType("int");

                    b.Property<int?>("RoleID")
                        .HasColumnType("int");

                    b.Property<int?>("UserID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("AssignerID");

                    b.HasIndex("RoleID");

                    b.HasIndex("UserID");

                    b.ToTable("UserRoles");
                });

            modelBuilder.Entity("SMS_backend.Models.UserRoleLog", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ID"));

                    b.Property<DateTime?>("UpdatedOn")
                        .HasColumnType("datetime2");

                    b.Property<int?>("UpdaterID")
                        .HasColumnType("int");

                    b.Property<int?>("UserRoleID")
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.HasIndex("UpdaterID");

                    b.HasIndex("UserRoleID");

                    b.ToTable("UserRoleLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Author", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorID");

                    b.Navigation("Creator");
                });

            modelBuilder.Entity("SMS_backend.Models.AuthorLog", b =>
                {
                    b.HasOne("SMS_backend.Models.Author", "Author")
                        .WithMany("AuthorLogs")
                        .HasForeignKey("AuthorID");

                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.Navigation("Author");

                    b.Navigation("Updater");
                });

            modelBuilder.Entity("SMS_backend.Models.Book", b =>
                {
                    b.HasOne("SMS_backend.Models.Author", "Author")
                        .WithMany("Books")
                        .HasForeignKey("AuthorID");

                    b.HasOne("SMS_backend.Models.User", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorID");

                    b.HasOne("SMS_backend.Models.Publisher", "Publisher")
                        .WithMany("Books")
                        .HasForeignKey("PublisherID");

                    b.Navigation("Author");

                    b.Navigation("Creator");

                    b.Navigation("Publisher");
                });

            modelBuilder.Entity("SMS_backend.Models.BookLog", b =>
                {
                    b.HasOne("SMS_backend.Models.Book", "Book")
                        .WithMany("BookLogs")
                        .HasForeignKey("BookID");

                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.Navigation("Book");

                    b.Navigation("Updater");
                });

            modelBuilder.Entity("SMS_backend.Models.Category", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorID");

                    b.Navigation("Creator");
                });

            modelBuilder.Entity("SMS_backend.Models.CategoryLog", b =>
                {
                    b.HasOne("SMS_backend.Models.Category", "Category")
                        .WithMany("CategoryLogs")
                        .HasForeignKey("CategoryID");

                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.Navigation("Category");

                    b.Navigation("Updater");
                });

            modelBuilder.Entity("SMS_backend.Models.Classroom", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Adviser")
                        .WithMany()
                        .HasForeignKey("AdviserID");

                    b.Navigation("Adviser");
                });

            modelBuilder.Entity("SMS_backend.Models.ClassroomLog", b =>
                {
                    b.HasOne("SMS_backend.Models.Classroom", "Classroom")
                        .WithMany("ClassroomLogs")
                        .HasForeignKey("ClassroomID");

                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.Navigation("Classroom");

                    b.Navigation("Updater");
                });

            modelBuilder.Entity("SMS_backend.Models.DailyConsumption", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Approver")
                        .WithMany()
                        .HasForeignKey("ApproverID");

                    b.HasOne("SMS_backend.Models.User", "Buyer")
                        .WithMany()
                        .HasForeignKey("BuyerID");

                    b.HasOne("SMS_backend.Models.Product", "Product")
                        .WithMany("DailyConsumptions")
                        .HasForeignKey("ProductID");

                    b.HasOne("SMS_backend.Models.ProductInventory", "ProductInventory")
                        .WithMany("DailyConsumptions")
                        .HasForeignKey("ProductInventoryID");

                    b.Navigation("Approver");

                    b.Navigation("Buyer");

                    b.Navigation("Product");

                    b.Navigation("ProductInventory");
                });

            modelBuilder.Entity("SMS_backend.Models.DailyConsumptionLog", b =>
                {
                    b.HasOne("SMS_backend.Models.DailyConsumption", "DailyConsumption")
                        .WithMany("DailyConsumptionLogs")
                        .HasForeignKey("DailyConsumptionID");

                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.Navigation("DailyConsumption");

                    b.Navigation("Updater");
                });

            modelBuilder.Entity("SMS_backend.Models.Department", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorID");

                    b.Navigation("Creator");
                });

            modelBuilder.Entity("SMS_backend.Models.DepartmentLog", b =>
                {
                    b.HasOne("SMS_backend.Models.Department", "Department")
                        .WithMany("DepartmentLogs")
                        .HasForeignKey("DepartmentID");

                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.Navigation("Department");

                    b.Navigation("Updater");
                });

            modelBuilder.Entity("SMS_backend.Models.Position", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorID");

                    b.HasOne("SMS_backend.Models.Department", "Department")
                        .WithMany("Positions")
                        .HasForeignKey("DepartmentID");

                    b.Navigation("Creator");

                    b.Navigation("Department");
                });

            modelBuilder.Entity("SMS_backend.Models.PositionLog", b =>
                {
                    b.HasOne("SMS_backend.Models.Position", "Position")
                        .WithMany("PositionLogs")
                        .HasForeignKey("PositionID");

                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.Navigation("Position");

                    b.Navigation("Updater");
                });

            modelBuilder.Entity("SMS_backend.Models.Product", b =>
                {
                    b.HasOne("SMS_backend.Models.Category", "Category")
                        .WithMany("Products")
                        .HasForeignKey("CategoryID");

                    b.HasOne("SMS_backend.Models.User", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorID");

                    b.Navigation("Category");

                    b.Navigation("Creator");
                });

            modelBuilder.Entity("SMS_backend.Models.ProductInventory", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorID");

                    b.HasOne("SMS_backend.Models.Product", "Product")
                        .WithMany("ProductInventories")
                        .HasForeignKey("ProductID");

                    b.Navigation("Creator");

                    b.Navigation("Product");
                });

            modelBuilder.Entity("SMS_backend.Models.ProductInventoryLog", b =>
                {
                    b.HasOne("SMS_backend.Models.ProductInventory", "ProductInventory")
                        .WithMany("ProductInventoryLogs")
                        .HasForeignKey("ProductInventoryID");

                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.Navigation("ProductInventory");

                    b.Navigation("Updater");
                });

            modelBuilder.Entity("SMS_backend.Models.ProductLog", b =>
                {
                    b.HasOne("SMS_backend.Models.Product", "Product")
                        .WithMany("ProductLogs")
                        .HasForeignKey("ProductID");

                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.Navigation("Product");

                    b.Navigation("Updater");
                });

            modelBuilder.Entity("SMS_backend.Models.Publisher", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorID");

                    b.Navigation("Creator");
                });

            modelBuilder.Entity("SMS_backend.Models.PublisherLog", b =>
                {
                    b.HasOne("SMS_backend.Models.Publisher", "Publisher")
                        .WithMany("PublisherLogs")
                        .HasForeignKey("PublisherID");

                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.Navigation("Publisher");

                    b.Navigation("Updater");
                });

            modelBuilder.Entity("SMS_backend.Models.RefreshToken", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "User")
                        .WithMany("RefreshTokens")
                        .HasForeignKey("UserID")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.Navigation("User");
                });

            modelBuilder.Entity("SMS_backend.Models.Role", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorID");

                    b.Navigation("Creator");
                });

            modelBuilder.Entity("SMS_backend.Models.RoleLog", b =>
                {
                    b.HasOne("SMS_backend.Models.Role", "Role")
                        .WithMany("RoleLogs")
                        .HasForeignKey("RoleID");

                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.Navigation("Role");

                    b.Navigation("Updater");
                });

            modelBuilder.Entity("SMS_backend.Models.Subject", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Teacher")
                        .WithMany()
                        .HasForeignKey("TeacherID");

                    b.Navigation("Teacher");
                });

            modelBuilder.Entity("SMS_backend.Models.SubjectLog", b =>
                {
                    b.HasOne("SMS_backend.Models.Subject", "Subject")
                        .WithMany("SubjectLogs")
                        .HasForeignKey("SubjectID");

                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.Navigation("Subject");

                    b.Navigation("Updater");
                });

            modelBuilder.Entity("SMS_backend.Models.User", b =>
                {
                    b.HasOne("SMS_backend.Models.Position", "Position")
                        .WithMany("Users")
                        .HasForeignKey("PositionID");

                    b.Navigation("Position");
                });

            modelBuilder.Entity("SMS_backend.Models.UserLog", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.HasOne("SMS_backend.Models.User", "User")
                        .WithMany("UserLogs")
                        .HasForeignKey("UserID");

                    b.Navigation("Updater");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SMS_backend.Models.UserRole", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Assigner")
                        .WithMany()
                        .HasForeignKey("AssignerID");

                    b.HasOne("SMS_backend.Models.Role", "Role")
                        .WithMany("UserRoles")
                        .HasForeignKey("RoleID");

                    b.HasOne("SMS_backend.Models.User", "User")
                        .WithMany("UserRoles")
                        .HasForeignKey("UserID");

                    b.Navigation("Assigner");

                    b.Navigation("Role");

                    b.Navigation("User");
                });

            modelBuilder.Entity("SMS_backend.Models.UserRoleLog", b =>
                {
                    b.HasOne("SMS_backend.Models.User", "Updater")
                        .WithMany()
                        .HasForeignKey("UpdaterID");

                    b.HasOne("SMS_backend.Models.UserRole", "UserRole")
                        .WithMany("UserRoleLogs")
                        .HasForeignKey("UserRoleID");

                    b.Navigation("Updater");

                    b.Navigation("UserRole");
                });

            modelBuilder.Entity("SMS_backend.Models.Author", b =>
                {
                    b.Navigation("AuthorLogs");

                    b.Navigation("Books");
                });

            modelBuilder.Entity("SMS_backend.Models.Book", b =>
                {
                    b.Navigation("BookLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Category", b =>
                {
                    b.Navigation("CategoryLogs");

                    b.Navigation("Products");
                });

            modelBuilder.Entity("SMS_backend.Models.Classroom", b =>
                {
                    b.Navigation("ClassroomLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.DailyConsumption", b =>
                {
                    b.Navigation("DailyConsumptionLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Department", b =>
                {
                    b.Navigation("DepartmentLogs");

                    b.Navigation("Positions");
                });

            modelBuilder.Entity("SMS_backend.Models.Position", b =>
                {
                    b.Navigation("PositionLogs");

                    b.Navigation("Users");
                });

            modelBuilder.Entity("SMS_backend.Models.Product", b =>
                {
                    b.Navigation("DailyConsumptions");

                    b.Navigation("ProductInventories");

                    b.Navigation("ProductLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.ProductInventory", b =>
                {
                    b.Navigation("DailyConsumptions");

                    b.Navigation("ProductInventoryLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Publisher", b =>
                {
                    b.Navigation("Books");

                    b.Navigation("PublisherLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.Role", b =>
                {
                    b.Navigation("RoleLogs");

                    b.Navigation("UserRoles");
                });

            modelBuilder.Entity("SMS_backend.Models.Subject", b =>
                {
                    b.Navigation("SubjectLogs");
                });

            modelBuilder.Entity("SMS_backend.Models.User", b =>
                {
                    b.Navigation("RefreshTokens");

                    b.Navigation("UserLogs");

                    b.Navigation("UserRoles");
                });

            modelBuilder.Entity("SMS_backend.Models.UserRole", b =>
                {
                    b.Navigation("UserRoleLogs");
                });
#pragma warning restore 612, 618
        }
    }
}
